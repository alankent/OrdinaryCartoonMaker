...
namespace VRM
{
    ...
    public class VRMLookAtHead : MonoBehaviour, IVRMComponent
    {
        ...
        void Awake()
        {
            if (Head == null) // AJK: added this
            {
                var animator = GetComponent<Animator>();
                if (animator == null)
                {
                    Debug.LogWarning("[VRMLookAtHead] animator is not found");
                    return;
                }
                ...
                if (head == null)
                {
                    Debug.LogWarning("[VRMLookAtHead] head is not found");
                    return;
                }
            ...
            //EditorApplication.update += DoPreview; // AJK - almost, but does not update correctly.
        }
        ...
        //void OnValidate() { Awake(); Update(); LateUpdate();  } // AJK
        //void OnDrawGizmosSelected() { Awake();  Update(); LateUpdate(); } // AJK
        //public void UMotionUpdate() { Awake(); Update(); LateUpdate(); } // AJK

        // AJK - this *almost* works - but it talks about the "Look rotation viewing vector is zero" and the eyes don't look in the correct direction.
        /*
        void DoPreview()
        {
            ...
            GetComponent<VRMLookAtBoneApplyer>().ApplyRotations(yaw, pitch);
        }*/ // AJK
        ...
        public void LookWorldPosition()
        {
            ...
            float pitch;
            LookWorldPosition(Target.position, out yaw, out pitch);
            //Debug.Log("[VRMLookAtHead] yaw=" + yaw + " pitch=" + pitch);
        }
