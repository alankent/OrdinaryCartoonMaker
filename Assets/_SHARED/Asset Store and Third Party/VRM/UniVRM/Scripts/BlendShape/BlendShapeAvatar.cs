...
namespace VRM
{
    ...
    public class BlendShapeAvatar : ScriptableObject
    {
        ...
        void Restore()
        {
            ...
            // AJK Was: Clips = Clips.OrderBy(x => BlendShapeKey.CreateFrom(x)).ToList();
            // Use default order for built in presets (01, 02, 03, etc), non-standard clip name otherwise.
            Clips = Clips.OrderBy(x => {
                var key = BlendShapeKey.CreateFrom(x);
                return (key.Preset == BlendShapePreset.Unknown) ? key.Name : ((int)key.Preset).ToString("00");
                }).ToList(); // AJK
        }
