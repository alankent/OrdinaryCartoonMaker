...
namespace UniHumanoid
{
    ...
    public class BvhSkeletonEstimator : ISkeletonDetector
    {
...
#else
            // AJK: VRoid uses these names always - skirt was getting it confused before.
            spine = hips.Traverse().First(x => x.Name.EndsWith("Spine"));
            leg_L = hips.Traverse().First(x => x.Name.EndsWith("L_UpperLeg") || x.Name.EndsWith("LeftUpperLeg"));
            leg_R = hips.Traverse().First(x => x.Name.EndsWith("R_UpperLeg") || x.Name.EndsWith("RightUpperLeg"));
#endif
        }
        ...
        static void GetNeckAndArms(IBone chest, out IBone neck, out IBone arm_L, out IBone arm_R)
        {
            ...
            {
                string s = "" + chest.Children.Count + " children: ";
                for (int i = 0; i < chest.Children.Count; i++)
                {
                    s += " " + i + "=" + chest.Children[i].ToString();
                }
                throw new System.Exception("Chest require 3 children, found" + s);
            }
...
#else
            neck = chest.Children.First(x => x.Name.EndsWith("Neck"));
            arm_L = chest.Children.First(x => x.Name.EndsWith("_L_Shoulder") || x.Name.EndsWith("LeftShoulder"));
            arm_R = chest.Children.First(x => x.Name.EndsWith("_R_Shoulder") || x.Name.EndsWith("RightShoulder"));
#endif
        }
        ...
        public Skeleton Detect(IList<IBone> bones)
        {
            ...
            // AJK var hips = root.Traverse().First(x => x.Children.Count == 3);
            var hips = root.Traverse().First(x => x.Name.EndsWith("Hips")); // Old rule was getting confused by skirt bones.
            Debug.Log("HIPS: " + hips.Name);
            ...
            {
                spineToChest.Add(x);
                Debug.Log("ADD: " + x.Name);
                if (x.Children.Count == 3) break;
            }
#else
            var spineToChest = new List<IBone>();
            var chest = spine.Children.First(x => x.Name == "Chest" || x.Name.EndsWith("_Chest"));
            var upperChest = chest.Children.First(x => x.Name.EndsWith("UpperChest"));
            spineToChest.Add(chest);
            spineToChest.Add(upperChest);
#endif
